...
- Party (vendor) is set as the creditor ledger, credited for the invoice total, with bill-wise allocation (`BILLTYPE="New Ref"`, named after the invoice number) so Tally tracks each invoice as its own outstanding bill rather than one lump balance per vendor — falls back to a date-based reference if the invoice number wasn't extracted
